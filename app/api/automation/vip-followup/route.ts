...
async function handle(req: Request) {
  ...
  try {
    ...
    const cutoff45d = new Date(Date.now() - 45 * 24 * 60 * 60 * 1000).toISOString();
    ...
    return NextResponse.json({ checked: vips.length, sent: results.filter((r) => r.sent).length, results });
  } catch (e) {
    const message = e instanceof Error ? e.message : "VIP follow-up failed";
